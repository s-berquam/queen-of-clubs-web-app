...
const TIP_TIERS = [
  { amount: 1, bonus: 15, label: "$1 — +15s" },
  { amount: 3, bonus: 45, label: "$3 — +45s" },
  { amount: 5, bonus: 90, label: "$5 — +90s" },
]
...
export default function QueuePage() {
  ...
  // Detect Square success redirect
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    if (params.get("selfie_success") === "true") {
      setSuccessPopup(true)
      window.history.replaceState({}, "", window.location.pathname)
      successTimerRef.current = setTimeout(() => setSuccessPopup(false), 4000)
    }
  ...
  return (
    <main className="queue-page" style={{ fontFamily: roboto.style.fontFamily }}>
      ...
      {successPopup && (
        <div className="success-popup">
          <div className="success-popup-inner">
            ...
            <button onClick={() => setSuccessPopup(false)}>Got it</button>
          </div>
        </div>
      )}
      ...
      {/* Tip modal */}
      {tipModalOpen && (
        <div className="modal-overlay" onClick={() => !tipping && setTipModalOpen(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <button className="modal-cancel" onClick={() => setTipModalOpen(false)} disabled={tipping}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {showBumpButton && (
        <div className="bump-banner">
          <span>Your selfie is on the big screen!</span>
          <button className="bump-btn" onClick={() => setTipModalOpen(true)}>
            Keep it on longer 🔥
          </button>
        </div>
      )}
      ...
      <div className="queue-list">
        {displayed.length === 0 && <p className="empty">No requests yet — be the first!</p>}
        {displayed.map((req, i) => (
      ...
      <style jsx>{`
        ...
        .bump-btn:hover { background: #ffc94d; }
        .vibe-filters {
        ...
        .filter-btn {
          ...
          color: #f0e6f5; font-size: 0.85rem; cursor: pointer; transition: all 0.2s;
        }
        ...
        .queue-card.up-next { border-color: #ffd77d; }
        ...
        .song-meta {
          font-size: 0.8rem; color: #c9b8e0; margin-top: 0.15rem;
          white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
        }
        .up-next-badge {
          background: #ffd77d; color: #333; font-size: 0.75rem;
          font-weight: bold; padding: 0.2rem 0.5rem;
          border-radius: 20px; white-space: nowrap;
        }
        .fire-btn {
          background: transparent; border: 2px solid #FF6F61;
          color: #FF6F61; border-radius: 20px; padding: 0.3rem 0.75rem;
          font-size: 0.9rem; cursor: pointer; transition: all 0.2s;
          white-space: nowrap; min-width: 60px; text-align: center;
        }
        .fire-btn:hover:not(:disabled) { background: #FF6F61; color: white; }
        .fire-btn.voted { background: #FF6F61; color: white; cursor: default; }
        .back-btn {
          display: block; max-width: 600px; margin: 1.5rem auto 0;
          width: 100%; padding: 0.75rem; border-radius: 12px; border: none;
          background-color: #FFDE59; color: #333; font-weight: bold;
        ...
        .modal h2 { font-size: 1.3rem; color: #f0e6f5; margin: 0; text-align: center; }
        ...
        .tip-tier-btn {
          ...
          background: #FF6F61; color: white; font-size: 1rem;
          font-weight: bold; cursor: pointer; transition: background 0.2s;
        }
        .tip-tier-btn:hover:not(:disabled) { background: #e85d50; }
        ...
        .success-popup-inner p { font-size: 1rem; color: #f0e6f5; margin: 0; }
        .success-popup-inner button {
          padding: 0.5rem 1.5rem; border-radius: 20px; border: none;
          background: #9effa3; color: #2c1a3b;
