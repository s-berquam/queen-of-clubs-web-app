...
import { SELFIE_TIP_TIER_MAP } from "lib/tiers"
...
export async function POST(req: NextRequest) {
  ...
  const tier = SELFIE_TIP_TIER_MAP[String(tipAmount)]
  if (!tier) {
  ...
  try {
    const url = await createPaymentLink({
      ...
      label: tier.apiLabel,
      metadata: { request_id: requestId, duration_bonus: String(tier.durationBonus) },
